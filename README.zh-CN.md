...
- **熔断**：基于 Resilience4j
- **负载均衡**：基于 Nacos 服务发现 + Spring Cloud LoadBalancer，支持按实例权重的一致性哈希与自动轮询降级
- **灰度路由**：金丝雀流量分流
...
├── gateway-loadbalancer  # 命名空间感知与一致性哈希负载均衡
├── gateway-gray          # 灰度 / 金丝雀路由
├── gateway-auth          # JWT 认证
├── gateway-transform     # 请求 / 响应转换
├── gateway-mirror        # 流量镜像
├── gateway-admin         # 配置管理 Admin REST API
└── namespace-demo-service# 本地多 namespace 路由演示服务
```
...
也可以直接通过 Gradle 启动网关；构建脚本会自动传入所需的 Java 25 preview 参数：
...
### 一致性哈希负载均衡（`aegis-governance.json`）

可按 Nacos `serviceId` 启用带权一致性哈希，以请求头或客户端 IP 作为会话粘滞键：
...
`keySource` 支持 `HEADER` 和 `CLIENT_IP`。使用 `CLIENT_IP` 时读取 `X-Forwarded-For` 的第一个地址，因此上游代理必须正确设置该请求头。`virtualNodesPerWeight` 默认为 160，并与 Nacos 实例权重共同决定虚拟节点数。服务未配置策略、无法提取哈希键或一致性哈希执行异常时，会 fail-open 降级为标准轮询。
